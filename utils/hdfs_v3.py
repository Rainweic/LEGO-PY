...
if os.path.isfile(cmd_local_file):
    ...
    result = subprocess.run(source_cmd_local, shell=True, check=True, stdout=subprocess.PIPE)
    output = result.stdout.decode('utf-8')
else:
    print(source_cmd_yarn)
    result = subprocess.run(source_cmd_yarn, shell=True, check=True, stdout=subprocess.PIPE)
    output = result.stdout.decode('utf-8')
...
class InsecureClient:
    ...
    def delete_remote_path(self, hdfs_path, obs_root_path):
        for file_path in self.fs.list_directory(hdfs_path):
            file_path_str = os.path.join('/', file_path['path'].replace(obs_root_path, ""))
            self.hdfs_rmr(file_path_str)
 
 
    def upload_single_file(self, local_path, hdfs_path, overwrite):
        print('upload_single_file===================== start time is:' + format(datetime.now()))
        hdfs_path_full = hdfs_path + '/' + os.path.basename(local_path)
        if not overwrite and self.fs.exists(hdfs_path_full):
            self.logger.error(f'the OBS target path {hdfs_path_full} is already existed as a file.', hdfs_path_full)
            return None
        ...
        self.logger.info('Upload local file %s to OBS %s succeeded.', local_path, hdfs_path)
        print('upload_single_file===================== end time is:' + format(datetime.now()))
        ...
        try:
            if os.path.exists(local_path):
                ...
                if not overwrite and os.path.exists(local_file):
                    raise ValueError('The target path ' + local_file +  ' is already exists.')
            ...
            self.logger.error('Download OBS file failed: ' + hdfs_path + 'Error: ' + str(e))
            return None
    ...
    def clear_folder(self, local_path, hdfs_path):
        ...
        for filename in self.fs.list_directory(hdfs_path_full):
            hdfs_list = os.path.join(hdfs_path_full, filename['name'].replace(self.bucket_name, '/'))
    ...
    def upload_folder_force(self, local_path, hdfs_path, overwrite):
        ...
        self.logger.info('force upload folder local_path %s to hdfs_path %s success.', local_path, hdfs_path)
        return None
    ...
    def download_folder(self, hdfs_path, local_path, overwrite):
        # 确保本地目录存在
        local_path_file_exists = os.path.normpath(local_path) + '/' + os.path.basename(hdfs_path)
        ...
        for filename in self.fs.list_directory(hdfs_path):
            hdfs_list = os.path.join(hdfs_path, filename['name'].replace(self.bucket_name, '/'))
            local_list = os.path.join(local_path_file_exists, os.path.basename(filename['name']))
            # 若是文件，直接下载
            if self.hdfs_path_isfile(hdfs_list):
                # 判断文件是否已存在，是否需要覆盖
                if os.path.exists(local_list) and overwrite:
                    os.remove(local_list)
                resp = self.download_single_file(hdfs_list, os.path.dirname(local_list), overwrite)
            # 若是文件夹，则递归下载
    ...
    def download_folder_force(self, hdfs_path, local_path, overwrite):
        ...
        self.logger.info('force download folder hdfs_path %s to local_path %s success.', hdfs_path, local_path)
        return None
...
def process_files(local_path: str, hdfs_path: str, obs_root_path: str, hdfs_user: str, overwrite: bool):
    global insecure_client
    if not insecure_client:
        insecure_client = InsecureClient(obs_root_path, _user=hdfs_user)
...
def process_if_overwrite(hdfs_path: str, obs_root_path: str, overwrite: bool, hdfs_user: str):
    global insecure_client
    if not insecure_client:
        insecure_client = InsecureClient(obs_root_path, _user=hdfs_user)
...
def multi_process_upload_folder(local_path: str, hdfs_path: str, obs_root_path: str, process_count: int,
                                overwrite: bool, hdfs_user: str):
    print('multi_process_upload_folder===================== start time is:' + format(datetime.now()))
    if process_count > os.cpu_count():
        raise ValueError("The number of process_count must be less than the number of CPU cores")
    ...
    with multiprocessing.Pool(processes=process_count) as pool:
        process_futures.append(pool.apply_async(process_if_overwrite, args=(hdfs_full_path, obs_root_path,
                                                                            overwrite, hdfs_user)))
    ...
    with multiprocessing.Pool(processes=process_count) as pool:
        dir_futures = []
        for root, dirs, files in os.walk(local_path):
            hdfs_dirs = [os.path.join(hdfs_path, local_base_name, root.replace(local_path, "").lstrip('/'), local_dir) for local_dir in dirs]
            local_dirs = [os.path.join(root, local_dir) for local_dir in dirs]
            for i in range(len(hdfs_dirs)):
                dir_futures.append(pool.apply_async(process_dirs, args=(local_dirs[i], hdfs_dirs[i],
                                                                        obs_root_path, hdfs_user)))
        # 等待所有任务完成
    ...
    with multiprocessing.Pool(processes=process_count) as pool:
        file_futures = []
        for root, dirs, files in os.walk(local_path):
            hdfs_file_path = os.path.join(hdfs_path, local_base_name, root.replace(local_path, "").lstrip('/'))
            local_files = [os.path.join(root, local_file) for local_file in files]
            for i in range(len(local_files)):
                file_futures.append(
                    pool.apply_async(process_files, args=(local_files[i], hdfs_file_path, obs_root_path, hdfs_user, overwrite)))
        # 等待所有任务完成
        for file_future in file_futures:
            file_future.get()
    print('multi_process_upload_folder===================== end time is:' + format(datetime.now()))
 
 
def process_download_folder(hdfs_path: str, hdfs_file_path: str, local_path: str, obs_root_path: str, overwrite: bool, hdfs_user: str):
    global insecure_client
    if not insecure_client:
        insecure_client = InsecureClient(obs_root_path, _user=hdfs_user)
 
    hdfs_relative_path = os.path.join(os.path.basename(hdfs_path), hdfs_file_path.replace(hdfs_path, "").lstrip("/"))
...
def multi_process_download_folder(hdfs_path: str, local_path: str, obs_root_path: str, process_count: int,
                                  overwrite: bool, hdfs_user: str):
    if process_count > os.cpu_count():
        raise ValueError("The number of process_count must be less than the number of CPU cores")
    ...
    with multiprocessing.Pool(processes=process_count) as pool:
        ...
        for hdfs_file in insecure_client_ins.fs.walk(hdfs_path):
            ...
            if hdfs_file['kind'] == 'directory':
                local_full_path = os.path.join(local_path_file, hdfs_file_path.replace(hdfs_path, '').lstrip("/"))
                if not os.path.exists(local_full_path):
                    os.makedirs(local_full_path)
            else:
                download_futures.append(pool.apply_async(process_download_folder, args=(hdfs_path, hdfs_file_path, local_path, obs_root_path, overwrite, hdfs_user)))
...
def upload(local_path: str, hdfs_path: str, obs_root_path: str, process_count: int, overwrite: bool, hdfs_user: str):
    logger = getLogger()
    ...
    if os.path.isfile(local_path):
        ...
        multi_process_upload_folder(local_path, hdfs_path, obs_root_path, process_count, overwrite, hdfs_user)
    logger.info('upload local path %s to hdfs path %s success.', local_path, hdfs_path)
    return None
 
 
def download(hdfs_path: str, local_path: str, obs_root_path: str, process_count: int, overwrite: bool, hdfs_user: str):
    logger = getLogger()
    ...
    if not insecure_client_ins.hdfs_path_exists(hdfs_path):
        logger.error('download hdfs_path is not exists,please check : %s ...', hdfs_path)
        return None
    ...
    if insecure_client_ins.hdfs_path_isfile(hdfs_path):
        ...
        multi_process_download_folder(hdfs_path, local_path, obs_root_path, process_count, overwrite, hdfs_user)
